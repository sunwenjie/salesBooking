class AdminsController < ApplicationController
  before_action :authenticate_user?, :except => [:forget_password, :reset_mail,:login,:logout]
  layout "login_appliction", :only => [:forget_password, :reset_mail]
  ...
  # def logout
  #  session[:user_id] = nil 
  #  redirect_to SendMenu::ReturnXmoUrls+"/admin/login"
  # end
 ...
 private
  ...
  def user_params
    params.require(:user).permit(:username, :name, :useremail, :usercontact, :user_status,:group_ids => [],:managers => [],:channel_ids=>[],:bu=>[])
  end

  def update_user_params
    params.require(:user).permit(:username, :name, :useremail, :usercontact)
  end
