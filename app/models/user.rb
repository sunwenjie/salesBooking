...
class User < Base
  ...
  # has_many :clients, inverse_of: :user,foreign_key: "created_user" , primary_key: "username"
  ...
  has_and_belongs_to_many :events, join_table: "#{SendMenu::DbConnection}.event_users"
  ...
  #validates_uniqueness_of :username,:allow_blank => true,conditions: -> { where.not(user_status: 'remove') }
  ...
  def share_operater_usernames
    shareoperater = ShareOperater.where("operater_name = ?",self.name).map(&:share_name)
  end

  def operater_usernames
    shareoperater = ShareOperater.where("share_name = ?",self.name).map(&:operater_name)
  end
  ...
  def self_bu_adcombo_new(product_type)
   if self.bu
    Product.where("product_type = ? and is_delete = ?",product_type,false).order("name").select{|product| ((product.bu.present? ? product.bu : []) & self.bu).size>0 }.collect{|product| [I18n.locale.to_s == "en"  ? product.en_name+ " ("+product.sale_model+")" : product.name + " ("+product.sale_model+")",product.id] }
   else
  ...
  def self_bu_adcombo_show(product_id)
    Product.where("id = ?",product_id).collect{|product| [(I18n.locale.to_s == "en" && product.en_name.present?) ? product.en_name + " ("+product.sale_model+")": product.name + " ("+product.sale_model+")",product.id] }
  end

  #个人BU产品类别
  def self_bu_adtype_new
   if self.bu
    Product.where("is_delete = ?",false).order("ad_platform","product_type").select{|product| ((product.bu.present? ? product.bu : []) & self.bu).size>0 }.collect{|product| [I18n.locale.to_s == "en"  ? product.product_type_en : product.product_type_cn,product.product_type] }.uniq << [ I18n.locale.to_s == "en" ? "Others" : "其他","OTHERTYPE"]
   else
