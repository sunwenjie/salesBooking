class OrderMailer < BaseMailer
  ...
  def notify_wait_order_examine(order, approval_groups, node_status, node_id, local_language, examination_id, resend= nil, planners = nil)
    @order = order
    @user = order.user
    @client = order.client
    ...
    if approval_groups.blank? && planners.present?
      deliver_emails = User.where("id in (?) and user_status != 'Stopped'", planners).map(&:useremail)
    else
