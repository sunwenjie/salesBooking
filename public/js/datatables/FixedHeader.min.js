...
}jQuery("thead tr th:gt(0)",b).remove();jQuery("tfoot tr th:gt(0)",b).remove();$("tbody tr",b).each(function(h){$("td:gt(0)",this).remove()
});this.fnEqualiseHeights("tbody",f.parentNode,b);var d=jQuery("thead tr th:eq(0)",c.nTable).outerWidth();
b.style.width=d+"px";g.nWrapper.style.width=d+"px"},_fnCloneTRight:function(g){var c=this.fnGetSettings();
...
if(c.bFooter){b.appendChild(jQuery("tfoot",c.nTable).clone(true)[0])}jQuery("thead tr th:not(:nth-child("+e+"n))",b).remove();
jQuery("tfoot tr th:not(:nth-child("+e+"n))",b).remove();$("tbody tr",b).each(function(h){$("td:lt("+(e-1)+")",this).remove()
});this.fnEqualiseHeights("tbody",f.parentNode,b);var d=jQuery("thead tr th:eq("+(e-1)+")",c.nTable).outerWidth();
b.style.width=d+"px";g.nWrapper.style.width=d+"px"},fnEqualiseHeights:function(e,d,g){var f=this,c=$(e+" tr:eq(0)",d).children(":eq(0)"),b=c.outerHeight()-c.height(),a=($.browser.msie&&($.browser.version=="6.0"||$.browser.version=="7.0"));
