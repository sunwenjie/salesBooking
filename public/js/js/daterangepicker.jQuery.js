...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			{text: 'Today', dateStart: 'today', dateEnd: 'today', value:"today"},
			{text: 'Last 7 days', dateStart: 'today-7days', dateEnd: 'today', value:"last_7_days" },
			{text: 'Month to date', dateStart: function(){ return Date.parse('today').moveToFirstDayOfMonth();  }, dateEnd: 'today', value:"month_to_date"},
			{text: 'Year to date', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today', value:"year_to_date" },
			//extras:
			{text: 'The previous Month', dateStart: function(){ return Date.parse('1 month ago').moveToFirstDayOfMonth();  }, dateEnd: function(){ return Date.parse('1 month ago').moveToLastDayOfMonth();  }, value:"the_previous_month" }
	...
	var rpPresets = (function(){
        ...
        if(this.value != undefined){
          jQuery('<li class="ui-daterangepicker-'+ this.text.replace(/ /g, '') +' ui-corner-all" period='+ this.value +'>'+ this.text +'</li>') 
		...
		jQuery.each(options.presets, function(key, value) {
			if (key === "dateRange"){
				jQuery('<li class="ui-daterangepicker-'+ key +' preset_'+ x +' ui-helper-clearfix ui-corner-all" period="all_days"><span class="ui-icon ui-icon-triangle-1-e"></span><a href="#">'+ value +'</a></li>')
				.appendTo(ul);
			}else{
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			doneBtn.hide();
			rpPickers.show();
			if($("#scheduler_btn").parent().hasClass('outer_submit'))
				$("#scheduler_btn").parent().hide();
			$("#scheduler_btn").hide();
