module Condition
  ...
  module ClassMethods
    ...
    def client_conditions(client)
      ...
      when String then { "#{clients}.name" => client }
      when Client then { "#{clients}.id" => client.id }
      else             { "#{clients}.id" => client.to_i }
      end
    end
    ...
    def user_conditions(user)
      ...
      when String then { "#{users}.name"  => user }
      when User then { "#{users}.id" => user.id }
      when Array then { "#{users}.id" => user }
      else             { "#{users}.id" => user.to_i }
      end
    end
    ...
    def industry_conditions(industry)
      ...
      when String then { "#{industries}.name" => industry }
      when Industry then { "#{industries}.id" => industry.id }
      else             { "#{industries}.id" => industry.to_i }
      end
    end
    ...
    def group_conditions(group)
      ...
      when String then { "#{groups}.group_name" => group }
      when Group then { "#{groups}.id" => group.id }
      when Array then { "#{groups}.id" => group }
      else             { "#{groups}.id" => group.to_i }
      end
    end
