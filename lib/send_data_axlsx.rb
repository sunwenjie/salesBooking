    ...
    my_package.workbook do |wb|
      ...
      wb.add_worksheet(:name => 'clients') do |ws|
        ...
        clients.each { |client|
          ...
          @ws.add_row [client.id, client.name, client.channel_name, share_users, client.created_at.present? ? client.created_at.localtime.strftime("%Y/%m/%d %H:%M") : '',client.state.blank? ? '-' : t("clients.index.#{client.state}")], :style => @table_default
        }
